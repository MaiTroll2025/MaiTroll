...
-- Rebuild TMV System (User Vehicles, Titles, Registrations, Insurance, Loans)
-- Cleans up old tables and establishes a server-authoritative flow.

-- 0. Deadlock Prevention (Attempt to kill other connections)
-- This is aggressive but necessary for a full rebuild in dev
DO $$
BEGIN
    ...
    WHERE pid <> pg_backend_pid()
      ...
      AND application_name <> 'supabase_admin'; -- specific to environment, usually just kill all others
EXCEPTION WHEN OTHERS THEN
    -- Ignore permission errors if we can't kill
    NULL;
END $$;

-- 1. Drop old tables if they exist (Clean Slate as requested)
DROP TABLE IF EXISTS public.vehicle_loans CASCADE;
...
-- Vehicle Loans (New for Unified Loans System)
...
CREATE POLICY "Users view own loans" ON public.vehicle_loans FOR SELECT USING (user_id = auth.uid());
...
-- 5b. Helper: Check Credit Eligibility (Added for Unified Loans)
...
-- 6. RPC: Purchase from KTAuto (Updated with License Check & Loans)
...
BEGIN
    ...
    -- 4. Check Purchase Limit (25 per rolling 30 days)
    ...
    -- 11. Create Insurance Policy (Unpaid)
    ...
    IF p_use_loan AND v_loan_amount > 0 THEN
        INSERT INTO public.vehicle_loans (
            ...
            v_loan_amount, 
            v_loan_amount, 
            CEIL(v_loan_amount / 12.0), -- 12 month term
    ...
    VALUES (
        ...
        -v_total_cost, 
        'purchase', 
        'Bought ' || v_car.name || ' from KTAuto' || (CASE WHEN p_use_loan THEN ' (Loan)' ELSE '' END)
    );
