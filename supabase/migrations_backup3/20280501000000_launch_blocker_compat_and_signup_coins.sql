-- Launch blocker compatibility fixes.
-- - Ensure concurrent refresh has the required unique index.
-- - Keep new-user welcome coins at exactly 250 going forward.
-- - Add user_reports alias columns used by older/newer frontend paths.
...
BEGIN
  IF EXISTS (
    ...
    WHERE schemaname = 'public'
      AND matviewname = 'user_earnings_summary'
  )
  AND EXISTS (
    SELECT 1
    ...
    EXECUTE 'CREATE UNIQUE INDEX IF NOT EXISTS idx_user_earnings_summary_user_id ON public.user_earnings_summary(user_id)';
  END IF;
END $$;
...
BEGIN
  IF to_regclass('public.user_reports') IS NOT NULL THEN
    IF NOT EXISTS (
      ...
      ALTER TABLE public.user_reports ADD COLUMN reported_user_id uuid;
    END IF;
    ...
    IF NOT EXISTS (
      ...
      ALTER TABLE public.user_reports ADD COLUMN reported_id uuid;
    END IF;
    ...
    EXECUTE 'CREATE INDEX IF NOT EXISTS idx_user_reports_reported_user_id ON public.user_reports(reported_user_id)';
    EXECUTE 'CREATE INDEX IF NOT EXISTS idx_user_reports_reported_id ON public.user_reports(reported_id)';
  END IF;
END $$;
...
BEGIN
  ...
  v_role := CASE WHEN v_email = 'Mai Troll2025@gmail.com' THEN 'admin' ELSE 'user' END;
  ...
  -- Non-critical: never let transaction logging roll back the profile row.
  BEGIN
