-- Fix gift/trollmond rules per new specification:
--
-- Rule 1: Gifts 1-99 coins cost — spend only coins, no trollmond deduction
-- Rule 2: Gifts >=100 coins per gift — spend coins AND deduct 100 trollmonds per gift
--         (only if sender has trollmonds; if sender is out of trollmonds, just pay full coin cost)
-- Rule 3: Trollmonds deducted from sender go to the receiver (broadcaster/seat/chat user)
-- Rule 4: Sender gets 10% of gift value back in coins (cashback) after gift is sent
-- Rule 5: Receiver gets FULL coins — no fees deducted from coins received
--
-- Additionally, the RPC now returns trollmonds_transferred so the frontend can
-- update the receiver's trollmond balance in real-time.

-- Add trollmonds_transferred column to stream_gifts if it doesn't exist
DO $$
BEGIN
  IF NOT EXISTS (
    ...
    WHERE table_schema = 'public'
      ...
      AND column_name = 'trollmonds_transferred'
  ) THEN
    ALTER TABLE public.stream_gifts
      ADD COLUMN trollmonds_transferred integer DEFAULT 0;
  END IF;
END $$;
...
BEGIN
  ...
  -- Rule: gifts >= 100 coins per gift cost 100 trollmonds per gift (if sender has trollmonds)
  -- Gifts under 100 coins cost 0 trollmonds
  ...
  -- If sender doesn't have enough trollmonds, deduct 0 trollmonds (they just pay coins)
  ...
  IF v_trollmonds_to_deduct > 0 THEN
    ...
    IF FOUND THEN
      -- Transfer deducted trollmonds to the receiver
      UPDATE public.user_profiles
      SET trollmonds = COALESCE(trollmonds, 0) + v_trollmonds_to_deduct
      WHERE id = p_receiver_id;
    ...
    ELSE
      -- Sender didn't have enough after all, skip trollmond deduction
      v_trollmonds_to_deduct := 0;
  ...
  INSERT INTO public.stream_gifts (
    ...
    v_total_cost,
    'coins',
    v_trollmonds_to_deduct,
    v_trollmonds_transferred,
    v_coins_back,
  ...
  -- Cashback transaction for sender
