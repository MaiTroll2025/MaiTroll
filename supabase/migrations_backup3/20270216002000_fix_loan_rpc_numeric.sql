-- Fix Loan Application RPC for Numeric Overflow
-- 1. Ensure loan_applications table supports numeric
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'loan_applications') THEN
        ALTER TABLE public.loan_applications ALTER COLUMN requested_coins TYPE NUMERIC USING requested_coins::NUMERIC;
    END IF;
END $$;

-- 2. Drop old versions of the RPC to resolve ambiguity
DROP FUNCTION IF EXISTS public.troll_bank_apply_for_loan(int);
DROP FUNCTION IF EXISTS public.troll_bank_apply_for_loan(uuid, int);

-- 3. Create new RPC with NUMERIC support
CREATE OR REPLACE FUNCTION public.troll_bank_apply_for_loan(
    p_requested_coins numeric,
    p_user_id uuid DEFAULT NULL -- Added for backward compatibility with older clients/edge functions
)
RETURNS json
LANGUAGE plpgsql
SECURITY DEFINER
...
DECLARE
    ...
    v_max_allowed numeric; -- Changed from bigint to numeric
    v_tier_name text;
    v_result json;
...
BEGIN
    ...
    -- Cast max_loan_coins to numeric to be safe
    SELECT max_loan_coins::numeric, tier_name INTO v_max_allowed, v_tier_name
    ...
    -- Disburse coins
    -- Calls the credit function internally (which should now be numeric-compatible)
