-- Fix two issues:
-- 1. send_gift_in_stream: gift lookup checked public.gifts (old store_items table) first,
--    causing store items to leak into broadcast gift flow. Fix: check gift_items first,
--    then purchasable_items (category=gift), then public.gifts as last resort.
-- 2. HytroGamingViewer handleTip sends tip item IDs (rose, troll_face, fire, crown, dragon,
...
BEGIN
  ...
  -- Look up gift cost — PRIMARY: gift_items (NOT public.gifts which is old store_items)
  SELECT value::integer, name
    INTO v_gift_cost, v_gift_name
  FROM public.gift_items
  WHERE id::text = p_gift_id OR gift_slug = p_gift_id OR name = p_gift_id
  LIMIT 1;
  ...
  -- ── Friday Battle Bonus (first gift per gifter per battle gets 5% back) ──
  IF v_battle_id IS NOT NULL THEN
...
-- ============================================================================
-- 3. SEED HYTROGAMING TIP ITEMS into gift_items
-- ============================================================================
-- HytroGamingViewer handleTip() sends these item IDs via send_gift_in_stream.
-- They need to exist in gift_items so the lookup succeeds.

INSERT INTO public.gift_items (id, name, icon, value, gift_slug, category, status, description)
VALUES
  (gen_random_uuid(), 'Rose',       '🌹', 10,   'tip_rose',       'gaming_tip', 'active', 'Gaming tip: Rose'),
  (gen_random_uuid(), 'Troll Face', '👹', 25,   'tip_troll_face', 'gaming_tip', 'active', 'Gaming tip: Troll Face'),
  (gen_random_uuid(), 'Fire',       '🔥', 50,   'tip_fire',       'gaming_tip', 'active', 'Gaming tip: Fire'),
  (gen_random_uuid(), 'Crown',      '👑', 100,  'tip_crown',      'gaming_tip', 'active', 'Gaming tip: Crown'),
  (gen_random_uuid(), 'Dragon',     '🐉', 250,  'tip_dragon',     'gaming_tip', 'active', 'Gaming tip: Dragon'),
  (gen_random_uuid(), 'Treasure',   '💎', 500,  'tip_treasure',   'gaming_tip', 'active', 'Gaming tip: Treasure')
