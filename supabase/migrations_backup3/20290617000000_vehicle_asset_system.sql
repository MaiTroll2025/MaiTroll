-- =====================================================
-- VEHICLE ASSET SYSTEM - Complete Migration
-- =====================================================
-- Cars Page (Bottom Navigation) - Vehicle Asset System
-- =====================================================
-- IMPORTANT: This system is INDEPENDENT from:
--   - Neighborhood System
--   - Starter Car System
...
BEGIN
  ...
  INSERT INTO public.vehicle_catalog (
    vehicle_id, name, tier, base_price, buyback_percentage,
    image_url, description, stock_quantity
  ) VALUES (
    p_vehicle_id, p_name, p_tier, p_base_price, p_buyback_percentage,
    p_image_url, p_description, p_stock_quantity
  )
...
AS $$
  SELECT
    ...
    COUNT(uva.id) FILTER (WHERE uva.status IN ('owned', 'sold')) as total_purchases,
    COUNT(uva.id) FILTER (WHERE uva.status = 'sold') as total_sales,
    COUNT(uva.id) FILTER (WHERE uva.status = 'owned') as currently_owned,
    COALESCE(SUM(uva.purchase_price) FILTER (WHERE uva.status IN ('owned', 'sold')), 0) as total_coins_spent,
...
-- SELECT * FROM public.get_vehicle_catalog();
