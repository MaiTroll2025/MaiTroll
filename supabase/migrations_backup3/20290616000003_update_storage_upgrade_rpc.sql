-- Update purchase_storage_upgrade RPC to include egress_included_bytes and egress_per_gb_cost
-- Also initialize replay balance on storage plan purchase
-- This ensures storage plans properly set egress allowance for viewer playback
...
DECLARE
    ...
    v_egress_cost INTEGER := 15; -- Default egress cost
BEGIN
    ...
    CASE p_tier_index
        WHEN 0 THEN v_egress_cost := 15; -- Starter
        WHEN 1 THEN v_egress_cost := 12; -- Basic
        WHEN 2 THEN v_egress_cost := 10; -- Standard
        WHEN 3 THEN v_egress_cost := 8;  -- Pro
        WHEN 4 THEN v_egress_cost := 6;  -- Premium
        WHEN 5 THEN v_egress_cost := 5;  -- Unlimited
    ...
    -- Initialize replay balance if not exists (gives 100 coins buffer for replay playback)
