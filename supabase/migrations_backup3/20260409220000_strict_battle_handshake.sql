...
RETURNS TRIGGER AS $$
BEGIN
  -- ❌ PREVENT SINGLE SIDE BATTLE START (only for strict handshake system)
  -- Skip these checks if battle is not using the strict handshake columns
  IF NEW.status = 'active' THEN
    -- Only enforce strict checks if this is actually a strict handshake battle
    -- (indicated by having team_a_stream_id or team_b_stream_id populated)
    IF (OLD.team_a_stream_id IS NOT NULL OR OLD.team_b_stream_id IS NOT NULL) THEN
  ...
  -- ❌ PREVENT DIRECT JUMP FROM WAITING TO ACTIVE (only for strict handshake system)
  -- Skip these checks if battle is not using the strict handshake columns
  IF (OLD.team_a_stream_id IS NOT NULL OR OLD.team_b_stream_id IS NOT NULL) THEN
  ...
  -- ✅ ALLOW ONLY CORRECT TRANSITIONS (only for strict handshake system)
  -- For non-strict handshake battles (e.g., instant battles, trollmers), allow any valid transition
  IF (OLD.team_a_stream_id IS NOT NULL OR OLD.team_b_stream_id IS NOT NULL) THEN
    IF NOT (
