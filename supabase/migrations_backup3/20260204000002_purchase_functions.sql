...
BEGIN
    ...
    -- Deduct coins
    SELECT success, message INTO v_success, v_msg FROM public.try_pay_coins(v_user_id, v_price, 'house_purchase', jsonb_build_object('house_id', p_house_catalog_id));
...
BEGIN
    ...
    -- Deduct coins
    SELECT success, message INTO v_success, v_msg FROM public.try_pay_coins(v_user_id, v_total_cost, 'car_purchase', jsonb_build_object('car_id', p_car_catalog_id));
