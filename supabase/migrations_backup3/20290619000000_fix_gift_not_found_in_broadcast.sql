-- Fix "Gift not found" when sending gifts in broadcast
-- The GiftBoxModal loads from gift_items and passes gift.id (UUID) to send_gift_in_stream.
-- The gift_items table has both `gift_slug` and `slug` columns (slug added by compat migration).
-- Ensure the RPC can match on either column, and also try `slug` as fallback.
...
BEGIN
  ...
  -- Match on id (UUID), gift_slug, slug, or name for maximum compatibility
  -- Use COALESCE(coin_cost, value) to support both new and legacy gift_items schemas
