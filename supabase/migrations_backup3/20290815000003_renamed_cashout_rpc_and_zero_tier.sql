-- Rename request_friday_cashout to request_cashout (MAI Pay fast, no Friday restriction)
-- Drop old function and any stale variants from prior migrations
DROP FUNCTION IF EXISTS public.request_friday_cashout(UUID, BIGINT, TEXT, TEXT, TEXT, TEXT);
DROP FUNCTION IF EXISTS public.request_cashout(UUID, BIGINT, TEXT, TEXT, TEXT, TEXT);
...
BEGIN
    ...
    -- Users may request multiple cashouts subject to the daily rolling limit enforced elsewhere.
...
-- Add $0 cashout tier
INSERT INTO public.cashout_tiers (coin_amount, cash_amount, currency, processing_fee_percentage, is_active, created_at)
VALUES (0, 0, 'USD', 0, true, NOW())
ON CONFLICT DO NOTHING;

-- ============================================================
-- Fix state_battle_mode column type drift
-- The live DB may have state_battle_mode as BOOLEAN (from an older
-- untracked migration), but all code and tracked migrations expect TEXT
-- with values 'none' | 'state'. This converts the column if needed.
-- ============================================================
DO $$
DECLARE
    v_col_type TEXT;
BEGIN
    ...
    IF v_col_type IS NOT NULL THEN
        IF v_col_type = 'boolean' THEN
            -- Convert BOOLEAN -> TEXT: true => 'state', false => 'none'
            ALTER TABLE public.streams
              ALTER COLUMN state_battle_mode TYPE TEXT
              USING CASE
                  WHEN state_battle_mode IS TRUE THEN 'state'
                  ELSE 'none'
              END;
            ALTER TABLE public.streams
    ...
    ELSE
        ...
        ALTER TABLE public.streams
          ADD COLUMN state_battle_mode TEXT DEFAULT 'none';
    END IF;
END $$;