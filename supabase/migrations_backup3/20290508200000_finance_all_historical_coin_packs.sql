-- Aggregate historical coin-pack / fiat purchases across coin_transactions hints,
-- paypal_transactions, and purchase_ledger for admin dashboards (all-time, no restrictive status filters).
...
BEGIN
  ...
  IF v_coin_kind_column IS NOT NULL
    ...
    AND EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'coin_transactions' AND column_name = 'metadata')
    AND v_money_sql <> ''
  THEN
    EXECUTE format($q$
    ...
    EXECUTE format($q$
      SELECT COALESCE(SUM(
        CASE
          ...
          WHEN EXISTS (SELECT 1 FROM information_schema.columns x
            ...
            CASE
              WHEN (
                EXISTS (SELECT 1 FROM information_schema.columns x
                       WHERE x.table_schema = 'public' AND x.table_name = 'coin_transactions' AND x.column_name = 'paypal_order_id')
  ...
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'paypal_transactions')
     AND EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'paypal_transactions' AND column_name = 'amount')
  THEN
    EXECUTE $sql$
  ...
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'purchase_ledger')
     AND EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'purchase_ledger' AND column_name = 'usd_amount')
     AND EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'purchasable_items')
     AND EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'purchasable_items' AND column_name = 'is_coin_pack')
  THEN
    EXECUTE $sql$
  ...
  ELSIF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'purchase_ledger')
     AND EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'purchase_ledger' AND column_name = 'usd_amount')
  THEN
    EXECUTE $sql$
      SELECT COALESCE(SUM(usd_amount), 0) FROM public.purchase_ledger WHERE usd_amount IS NOT NULL
    $sql$ INTO v_coin_sales_ledger;
  END IF;
  ...
  IF v_coin_sales_revenue = 0 AND EXISTS (
    SELECT 1 FROM information_schema.columns
