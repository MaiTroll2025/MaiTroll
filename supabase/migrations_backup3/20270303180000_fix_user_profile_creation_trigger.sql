...
BEGIN
  ...
  -- Role: prioritize frontend-supplied role, fall back to email-based default
  v_role := COALESCE(
    NEW.raw_user_meta_data->>'role',
    CASE WHEN LOWER(NEW.email) = 'Mai Troll2025@gmail.com' THEN 'admin'::text ELSE 'user'::text END
  );

  -- Organization: use provided org_id from signup if present (verified server-side)
...
-- Uses ID-based usernames to guarantee uniqueness
 ...
 SELECT 
   ...
   CASE 
     WHEN LOWER(au.email) = 'Mai Troll2025@gmail.com' THEN 'admin'
