-- Fix send_gift lookup to use gift_items / purchasable_items (not public.gifts)
-- This is a permanent fix for "invalid gift" errors when sending broadcast gifts
...
BEGIN
    ...
    -- 2. Get Gift Details
    -- Primary: gift_items
    SELECT COALESCE(coin_cost, value), name INTO v_cost, v_name
    FROM public.gift_items
    WHERE id = p_gift_id;

    -- Fallback: purchasable_items (category: gift)
    IF v_cost IS NULL THEN
        SELECT coin_price, display_name INTO v_cost, v_name
