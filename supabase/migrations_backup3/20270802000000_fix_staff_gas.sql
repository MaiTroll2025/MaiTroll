-- Fix Staff Gas Logic to ensure they are always at 100% and pay nothing
-- Applies to: admin, secretary, troll_officer, lead_troll_officer

-- 1. Update refill_gas to be free for staff
...
BEGIN
    ...
    IF v_is_staff THEN
        v_new_gas := 100.0;
    ELSE
        v_new_gas := LEAST(COALESCE((SELECT gas_balance FROM public.user_profiles WHERE id = v_user_id), 0) + p_amount_percent, 100.0);
    END IF;

    UPDATE public.user_profiles
    SET gas_balance = v_new_gas,
        last_gas_update = now()
    WHERE id = v_user_id;
...
-- 2. Update consume_gas to grant unlimited gas to staff
