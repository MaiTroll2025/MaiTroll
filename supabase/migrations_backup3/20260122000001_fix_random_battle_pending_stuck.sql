-- Fix random battles being stuck at 'pending': immediately mark the battle active
-- in find_random_battle_match after both participants are confirmed. If the
-- downstream activate_random_battle RPC never fires (e.g. broadcaster navigated
-- away), the battle was previously left at 'pending' and never progressed.
--
...
BEGIN
  ...
  -- Immediately mark the battle active after both participants are confirmed.
  -- This prevents the battle from being stuck at 'pending' if the downstream
  -- activate_random_battle RPC never fires (e.g. broadcaster navigated away).
