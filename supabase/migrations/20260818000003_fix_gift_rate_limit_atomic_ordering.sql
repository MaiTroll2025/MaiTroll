-- ============================================
-- FIX: Atomic gift rate limit ordering + idempotency + recruiter
-- ============================================

-- Ensure gift_rate_limits table exists (created in prior migration)
...
-- Ensure txn_key column exists on stream_gifts (created in prior migration)
...
-- ============================================
-- CORRECTED send_gift_in_stream
-- Atomic ordering:
--   1. Lock sender row
--   2. Validate inputs
--   3. Check idempotency (before rate limit so duplicates don't count)
--   4. Check rate limit (10 gift sends per minute)
--   5. Check balance
--   6. Calculate splits
--   7. Deduct coins / trollmonds
--   8. Credit receiver, agency, leader, recruiter
--   9. Record stream_gifts (with ON CONFLICT safety net)
--  10. Record rate limit (only after success)
--  11. Return
-- ============================================
...
BEGIN
  ...
  -- ============================================
  -- Step 1: Lock sender row FIRST to serialize all
  -- concurrent operations on this user
  -- ============================================
  ...
  -- ============================================
  -- Step 3: Check idempotency BEFORE rate limit
  -- Duplicates must not consume rate limit quota
  -- ============================================
  IF v_txn_key IS NOT NULL THEN
    SELECT id::text INTO v_existing_id
    FROM public.stream_gifts
    WHERE sender_id = p_sender_id
      AND txn_key = v_txn_key
    LIMIT 1;
  ...
  -- ============================================
  -- Step 4: Rate limit — 10 gift SENDS per rolling minute
  -- Note: p_quantity is the number of gifts in this send.
  -- The rate limit counts sends, not individual gifts.
  -- ============================================
  SELECT count(*)
  ...
  -- Determine trollmond deduction (100 trollmonds per gift if gift_cost >= 100)
  ...
  -- ============================================
  -- Step 7: Calculate agency splits
  -- ============================================
  SELECT am.agency_id,
         a.status,
  ...
  -- ============================================
  -- Step 8: Deduct coins from sender
  -- ============================================
  ...
  -- ============================================
  -- Step 9: Deduct trollmonds and transfer to receiver
  -- ============================================
  ...
  -- ============================================
  -- Step 10: Credit receiver with full coins
  -- ============================================
  UPDATE public.user_profiles
  ...
  -- ============================================
  -- Step 12: Record stream_gifts
  -- Use ON CONFLICT as safety net for idempotency
  -- ============================================
  ...
  -- ============================================
  -- Step 15: Leader/recruiter bonus credit
  -- ============================================
  IF v_leader_bonus > 0 AND v_leader_user_id IS NOT NULL THEN
    UPDATE public.user_profiles
  ...
  -- ============================================
  -- Step 16: Coin transactions for sender and receiver
  -- ============================================
  ...
  -- ============================================
  -- Step 19: Friday battle bonus (optional, fail-safe)
  -- ============================================
  BEGIN
    IF v_battle_id IS NOT NULL THEN
