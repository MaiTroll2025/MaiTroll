-- Fix: Ensure send_gift_in_stream always returns and never hits 2F005
-- Error: SQL state 2F005 (control_reached_end_of_function_without_return)
-- Root cause: The award_friday_battle_gifter_bonus() call is not wrapped in an
-- exception handler; if the function is missing or errors, the outer function
-- exits without reaching its RETURN statement. Also added a top-level
-- EXCEPTION handler as a safety net to always return valid JSON.
...
BEGIN
  ...
  -- Prevent self-gifting: all gifts must go to other users
  ...
  -- so that a missing or failing award_friday_battle_gifter_bonus function
  -- does not prevent the mandatory final RETURN (Bug: 2F005)
  BEGIN
    IF v_battle_id IS NOT NULL THEN
