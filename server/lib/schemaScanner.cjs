...
const SQL_KEYWORDS = new Set([
  ...
  'simple', 'partial', 'exclusion', 'include', 'tablespace',
])
...
function extractTableFromQuery(text) {
  const patterns = [
...
function scanMigrations() {
  ...
  for (const fullPath of SQL_FILES) {
    let content
    try {
      content = fs.readFileSync(fullPath, 'utf-8')
    } catch {
    ...
    for (const line of lines) {
      ...
      const grantMatch = unquoted.match(/GRANT\s+.*\s+ON\s+FUNCTION\s+(?:public\.)?([a-z_][a-z0-9_]*)\(/i)
      if (grantMatch) {
