...
function PhoneJailRedirect({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    void checkJailStatus()

    const channel = supabase
      .channel(`phone-jail:${user.id}`)
      .on(
        'postgres_changes',
        {
          event: '*',
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-black text-white">
        <div className="text-center">
          <div className="mx-auto mb-4 h-10 w-10 animate-spin rounded-full border-2 border-red-500/20 border-t-red-500" />
