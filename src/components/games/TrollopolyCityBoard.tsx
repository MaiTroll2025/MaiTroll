...
import { 
  ...
  Dice5, Home, Coins, Moon, Sun, MapPin, Repeat2, Menu, Landmark
} from 'lucide-react';
...
interface TrollopolyCityBoardProps {
  ...
  isSpectator?: boolean;
  streamId?: string;
  playerMedia?: Record<string, { videoTrack?: any; audioTrack?: any; isLocal?: boolean }>;
}

function LiveKitMiniVideo({ media, fallbackInitial, color }: { media?: { videoTrack?: any; isLocal?: boolean }; fallbackInitial: string; color: string }) {
  ...
  useEffect(() => {
    ...
    videoElement.muted = !!media?.isLocal;
    ...
    return () => {
      try {
        ...
        console.warn('[TrollopolyCityBoard] Failed to detach video track:', err);
      }
      if (container.contains(videoElement)) {
        container.removeChild(videoElement);
      }
    };
  }, [media?.videoTrack, media?.isLocal]);

  return (
    <div ref={containerRef} className="relative w-full h-full bg-black overflow-hidden">
      {!media?.videoTrack && (
        <div className="flex items-center justify-center w-full h-full bg-slate-800">
          <div className="w-16 h-16 rounded-full flex items-center justify-center text-2xl font-bold" style={{ backgroundColor: color }}>
            {fallbackInitial}
          </div>
        </div>
      )}
...
const getRoadPosition = (tileIndex: number): { x: number; z: number; rotation: number } => {
  ...
  switch (side) {
    ...
    case 1: // Left side (jail to park) - bottom to top
      return {
        x: -roadOffset,
        z: roadOffset - CORNER_TILE_SIZE - indexOnSide * step,
        rotation: Math.PI,
      };
    case 2: // Top side (park to jail transport) - left to right
      return {
        x: -roadOffset + CORNER_TILE_SIZE + indexOnSide * step,
        z: -roadOffset,
        rotation: -Math.PI / 2,
      };
    case 3: // Right side (jail transport to city center) - top to bottom
      return {
        x: roadOffset,
        z: -roadOffset + CORNER_TILE_SIZE + indexOnSide * step,
        rotation: 0,
...
const getCornerPosition = (cornerIndex: number): { x: number; z: number } => {
  ...
  switch (cornerIndex) {
    ...
    case 2: return { x: -offset, z: -offset }; // Public park (top left)
    case 3: return { x: offset, z: -offset }; // Jail transport (top right)
...
export const TrollopolyCityBoard: React.FC<TrollopolyCityBoardProps> = ({
  ...
  const diceStateRef = useRef(gameState.dice);
  const boardIsDayRef = useRef(true);
  ...
  const [pendingTileAction, setPendingTileAction] = useState(false);
  const [modalTile, setModalTile] = useState<Property | null>(null);
  const [displayDice, setDisplayDice] = useState({ die1: gameState.dice.die1, die2: gameState.dice.die2 });
  const [modActionTarget, setModActionTarget] = useState<{ id: string; username: string; avatar_url: string } | null>(null);
  const previousPositionsRef = useRef<Map<string, number>>(new Map());

  const activePlayer = gameState.players[gameState.currentPlayerIndex];
  const isCurrentTurn = gameState.players[gameState.currentPlayerIndex]?.id === playerId && !isSpectator;
  const currentTurnUserId = activePlayer?.id;
  const isRolling = gameState.dice.isRolling;
  const isMoving = movingVehicle !== null;
  const lastRoll = gameState.dice.die1 + gameState.dice.die2;
  const canModerateGameChat = isHost || isStaffProfile(profile);
  ...
  useEffect(() => {
    diceStateRef.current = gameState.dice;
  }, [gameState.dice]);

  useEffect(() => {
    if (!gameState.dice.isRolling) {
      setDisplayDice({ die1: gameState.dice.die1, die2: gameState.dice.die2 });
      return;
    }
  ...
  }, [gameState.dice.die1, gameState.dice.die2, gameState.dice.isRolling]);
  ...
  useEffect(() => {
    ...
    renderer.toneMappingExposure = isDay ? 1 : 1.25;
    ...
    const animate = () => {
      ...
      const diceState = diceStateRef.current;
      if (diceRef.current && diceState.isRolling) {
        ...
        setDiceRotation(diceRef.current.die1, diceState.die1);
        setDiceRotation(diceRef.current.die2, diceState.die2);
  ...
  const setupLighting = (scene: THREE.Scene, isDay: boolean) => {
    ...
    if (!isDay) {
      ...
      const moonMaterial = new THREE.MeshBasicMaterial({ color: 0xffffdd });
      ...
      const boardGlow = new THREE.PointLight(0x22d3ee, 1.8, 36);
  ...
  const createRoads = (scene: THREE.Scene) => {
    const asphaltMaterial = new THREE.MeshStandardMaterial({ color: 0x161922, roughness: 0.88, metalness: 0.08 });
    const isDay = boardIsDayRef.current;
    const lineMaterial = new THREE.MeshBasicMaterial({ color: 0xffcc00, transparent: true, opacity: isDay ? 0.75 : 1 });
    const whiteLineMaterial = new THREE.MeshBasicMaterial({ color: isDay ? 0xffffff : 0x8ee8ff, transparent: true, opacity: isDay ? 0.75 : 0.95 });
    const neonMaterial = new THREE.MeshBasicMaterial({ color: 0x22d3ee, transparent: true, opacity: isDay ? 0.18 : 0.55 });
    ...
    [-ROAD_WIDTH / 2 - 0.08, ROAD_WIDTH / 2 + 0.08].forEach(offset => {
      const bottomGlow = new THREE.Mesh(new THREE.PlaneGeometry(sideLength, 0.08), neonMaterial);
      bottomGlow.rotation.x = -Math.PI / 2;
      bottomGlow.position.set(0, 0.035, ROAD_RADIUS - ROAD_WIDTH / 2 + offset);
      scene.add(bottomGlow);

      const topGlow = bottomGlow.clone();
      topGlow.position.set(0, 0.035, -ROAD_RADIUS + ROAD_WIDTH / 2 + offset);
      scene.add(topGlow);

      const leftGlow = new THREE.Mesh(new THREE.PlaneGeometry(0.08, sideLength), neonMaterial);
      leftGlow.rotation.x = -Math.PI / 2;
      leftGlow.position.set(-ROAD_RADIUS + ROAD_WIDTH / 2 + offset, 0.035, 0);
      scene.add(leftGlow);

      const rightGlow = leftGlow.clone();
      rightGlow.position.set(ROAD_RADIUS - ROAD_WIDTH / 2 + offset, 0.035, 0);
      scene.add(rightGlow);
    });
  };
  ...
  const createLabelTexture = (title: string, subtitle?: string, accent = '#22d3ee') => {
    ...
    lines.slice(0, 3).forEach((text, index) => {
      ctx.fillText(text, canvas.width / 2, 76 + index * 42);
    });

    if (subtitle) {
      ctx.font = 'bold 30px Arial';
      ctx.fillStyle = '#664d10';
      ctx.fillText(subtitle, canvas.width / 2, 218);
    }
  ...
  const addTileLabel = (group: THREE.Group, property: Property, size = TILE_SIZE) => {
    const texture = createLabelTexture(
      property.name,
      property.price > 0 ? property.price.toLocaleString() : undefined,
      property.color
    );
    if (!texture) return;

    const label = new THREE.Mesh(
      new THREE.PlaneGeometry(size * 0.92, size * 0.58),
  ...
  const createTiles = (scene: THREE.Scene) => {
    gameState.properties.forEach((property, index) => {
      ...
      const borderMaterial = new THREE.MeshBasicMaterial({ color: boardIsDayRef.current ? 0x333333 : property.color });
      const border = new THREE.Mesh(borderGeometry, borderMaterial);
      border.position.y = 0.01;
      group.add(border);
      addTileLabel(group, property);
  ...
  const createCornerTiles = (scene: THREE.Scene) => {
    ...
    cornerIndices.forEach((index, i) => {
      ...
      border.position.y = 0.01;
      group.add(border);
      if (property) addTileLabel(group, property, CORNER_TILE_SIZE);
  ...
  const createWindowMaterial = (baseColor = 0x87ceeb) => new THREE.MeshStandardMaterial({
    color: boardIsDayRef.current ? baseColor : 0xfff2a0,
    emissive: boardIsDayRef.current ? 0x000000 : 0xffd45a,
    emissiveIntensity: boardIsDayRef.current ? 0 : 0.85,
    roughness: boardIsDayRef.current ? 0.25 : 0.15,
  });

  const addPropertySign = (group: THREE.Group, property: Property, y: number) => {
    const texture = createLabelTexture(property.name, property.price > 0 ? property.price.toLocaleString() : undefined, property.color);
    if (!texture) return;

    const sign = new THREE.Mesh(
      new THREE.PlaneGeometry(1.45, 0.55),
      new THREE.MeshBasicMaterial({ map: texture, transparent: true })
    );
    sign.position.set(0, y, 0.83);
    group.add(sign);

    if (!boardIsDayRef.current) {
      const light = new THREE.PointLight(new THREE.Color(property.color), 0.7, 4);
      light.position.set(0, y, 1.1);
      group.add(light);
    }
  };

  const addRoofDetails = (group: THREE.Group, color: string | number, y: number) => {
    const cap = new THREE.Mesh(
      new THREE.BoxGeometry(1.35, 0.08, 1.05),
      new THREE.MeshStandardMaterial({ color, metalness: 0.35, roughness: 0.32 })
    );
    cap.position.y = y;
    group.add(cap);
    ...
    const antenna = new THREE.Mesh(
      ...
      new THREE.MeshStandardMaterial({ color: 0xb9c6d4, metalness: 0.8, roughness: 0.2 })
    );
    antenna.position.set(0.35, y + 0.38, -0.2);
    group.add(antenna);
  };
  ...
  const createHouseBuilding = (group: THREE.Group, property: Property) => {
    ...
    addPropertySign(group, property, 1.65);
  };
  ...
  const createApartmentBuilding = (group: THREE.Group, property: Property) => {
    ...
    addRoofDetails(group, property.color, 0.15 + floors * floorHeight + 0.08);
    addPropertySign(group, property, 3.25);
  };
  ...
  const createSkyscraperBuilding = (group: THREE.Group, property: Property) => {
    ...
    // Window grid
    const windowMat = createWindowMaterial(0xdbeafe);
    ...
    group.add(spire);
    addRoofDetails(group, property.color, 0.15 + floors * floorHeight + 0.08);
    addPropertySign(group, property, 4.8);
  };
  ...
  const moveVehicleAlongRoad = (playerId: string, fromTile: number, toTile: number, onComplete?: () => void) => {
    ...
    const moveStep = () => {
      ...
      const duration = Math.max(30, Math.min(120, Math.floor(1200 / Math.max(path.length, 1))));
  ...
  useEffect(() => {
    ...
    gameState.players.forEach((player) => {
      if (player.position !== undefined) {
        const previousPosition = previousPositionsRef.current.get(player.id) ?? 0;

        if (previousPosition !== player.position) {
          setPendingTileAction(true);
          setSpaceModalOpen(false);
          moveVehicleAlongRoad(player.id, previousPosition, player.position, () => {
            const landedTile = gameState.properties[player.position];
            setSelectedTile(player.position);
            setModalTile(landedTile || null);
            setPendingTileAction(false);
            setSpaceModalOpen(!!landedTile);
          });
        }

        previousPositionsRef.current.set(player.id, player.position);
      }
    });
  }, [gameState.players.map(p => p.position).join(','), isRolling]);
  ...
  const createBackgroundCity = (scene: THREE.Scene) => {
    ...
    for (let i = 0; i < 25; i++) {
      ...
      if (!boardIsDayRef.current && i % 2 === 0) {
        const windowRows = Math.min(8, Math.floor(h / 2));
        for (let row = 0; row < windowRows; row++) {
          const lightBand = new THREE.Mesh(
            new THREE.PlaneGeometry(2.8, 0.16),
            new THREE.MeshBasicMaterial({ color: 0xffd36a, transparent: true, opacity: 0.45 })
          );
          lightBand.position.set(x, 1.5 + row * 1.7, z + 2.05);
          scene.add(lightBand);
        }
      }
    }
  ...
  const createCenterCity = (scene: THREE.Scene) => {
    ...
    const ringMaterial = new THREE.MeshBasicMaterial({ color: 0x38bdf8, transparent: true, opacity: boardIsDayRef.current ? 0.25 : 0.75 });
    [3.6, 5.2, 7.1].forEach(radius => {
      const ring = new THREE.Mesh(new THREE.RingGeometry(radius, radius + 0.05, 96), ringMaterial);
      ring.rotation.x = -Math.PI / 2;
      ring.position.y = 0.04;
      scene.add(ring);
    });

    const fountainLight = new THREE.PointLight(0x38bdf8, boardIsDayRef.current ? 0.5 : 1.4, 16);
    fountainLight.position.set(0, 3, 0);
  ...
  const handleRollDice = () => {
    if (currentTurnUserId !== playerId || isSpectator || gameState.phase !== 'waiting_for_roll' || isRolling || isMoving || pendingTileAction || spaceModalOpen) return;
    setSpaceModalOpen(false);
    setModalTile(null);
  ...
  const currentProperty = activePlayer ? gameState.properties[activePlayer.position] : null;
  const visibleTile = modalTile || currentProperty;

  useEffect(() => {
    if (currentProperty && !isRolling && !isMoving && pendingTileAction) {
      setModalTile(currentProperty);
  ...
  return (
    <div className="relative w-full h-screen bg-slate-900 overflow-y-auto overflow-x-hidden md:overflow-hidden">
      ...
      <div className="absolute top-4 left-1/2 -translate-x-1/2 z-30 flex flex-col items-center gap-2">
        <div className="px-6 py-2 rounded-2xl bg-black/75 border border-cyan-400/40 shadow-2xl shadow-cyan-500/20">
          <div className="text-3xl font-black text-white tracking-wide leading-none">TROLLOPOLY</div>
        ...
        <div className="flex items-center gap-2 px-4 py-2 bg-slate-950/90 rounded-full border border-slate-600">
          ...
          <span className="text-slate-500">|</span>
          <span className="text-white text-sm">{Math.floor((gameState.timerRemaining || 0) / 60).toString().padStart(2, '0')}:{((gameState.timerRemaining || 0) % 60).toString().padStart(2, '0')}</span>
          <span className="text-slate-500">|</span>
          <Eye size={16} className="text-slate-400" />
          <span className="text-white text-sm">{gameState.spectatorCount}</span>
          {isDayTime ? (
      ...
      {/* Player Cameras */}
      <div className="absolute left-4 top-24 bottom-36 z-20 w-[230px] overflow-y-auto pr-2 space-y-3">
      {gameState.players.slice(0, 6).map((player, index) => {
        ...
        return (
          <div key={player.id} className="relative transition-all duration-300">
            <div className={`relative bg-slate-950/95 rounded-xl overflow-hidden border-2 ${gameState.currentPlayerIndex === index ? 'border-cyan-400 shadow-lg shadow-cyan-400/30' : isHighlighted ? 'border-yellow-400 shadow-lg shadow-yellow-400/50' : 'border-slate-600'} transition-all duration-300`} style={{ width: '220px', height: '148px' }}>
              <div className="relative w-full h-full bg-black">
                {cameraState.enabled ? (
                  <LiveKitMiniVideo
                    media={playerMedia[player.id]}
                    fallbackInitial={player.username.charAt(0).toUpperCase()}
      ...
      {/* Spectator Count */}
      <div className="hidden absolute top-16 left-1/2 -translate-x-1/2 z-20">
      ...
      {/* Chat Panel */}
      <div className={`absolute right-4 top-24 bottom-36 z-20 transition-all duration-300 ${showChat ? 'translate-x-0' : 'translate-x-[calc(100%+1rem)]'}`}>
        <div className="w-80 max-w-[calc(100vw-2rem)] h-full bg-slate-950/95 rounded-xl border border-slate-600 overflow-hidden shadow-2xl flex flex-col">
                ...
                {msg.type === 'system' ? (
                  ...
                  <>
                    <button
                      type="button"
                      onClick={() => {
                        if (!canModerateGameChat) return;
                        setModActionTarget({
                          id: msg.user_id,
                          username: msg.sender_name || msg.user_profiles?.username || msg.user_profiles?.display_name || 'Troll Citizen',
                          avatar_url: msg.user_profiles?.avatar_url || '',
                        });
                      }}
                      className={`font-medium text-blue-400 ${canModerateGameChat ? 'hover:underline' : 'cursor-default'}`}
                    >
                      {msg.sender_name || msg.user_profiles?.username || msg.user_profiles?.display_name || 'Troll Citizen'}:
                    </button>{' '}
                    <span>{msg.content}</span>
                  </>
                )}
      ...
      {!showChat && (
        <button onClick={() => setShowChat(true)} className="absolute right-4 top-24 z-20 p-3 bg-slate-800 hover:bg-slate-700 rounded-full border border-slate-600 text-white transition-all shadow-lg">
          <MessageSquare size={20} />
        </button>
      )}


      {visibleTile && spaceModalOpen && !isRolling && !isMoving && (
        <div className="absolute inset-0 z-30 flex items-center justify-center pointer-events-none px-4">
          <div className="pointer-events-auto w-full max-w-sm bg-slate-900/95 rounded-2xl border border-slate-500 shadow-2xl overflow-hidden">
            <div className="h-2" style={{ backgroundColor: visibleTile.color }} />
            <div className="p-5">
              <div className="flex items-start justify-between gap-3 mb-3">
                ...
                <button onClick={() => setSpaceModalOpen(false)} className="p-1.5 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-300">
                  x
                </button>
              </div>
              <div className="grid grid-cols-2 gap-3 text-sm">
                <div className="rounded-lg bg-slate-800/80 p-3">
                  <p className="text-slate-500 uppercase text-[10px] font-bold">Type</p>
                  <p className="text-white capitalize">{visibleTile.type}</p>
                </div>
                <div className="rounded-lg bg-slate-800/80 p-3">
                  <p className="text-slate-500 uppercase text-[10px] font-bold">Price</p>
                  <p className="text-yellow-400 font-bold">{visibleTile.price > 0 ? `${visibleTile.price.toLocaleString()} TC` : 'Special'}</p>
                </div>
                <div className="rounded-lg bg-slate-800/80 p-3">
                  <p className="text-slate-500 uppercase text-[10px] font-bold">Rent</p>
                  <p className="text-emerald-400 font-bold">{(visibleTile.baseRent || 0).toLocaleString()} TC</p>
                </div>
                <div className="rounded-lg bg-slate-800/80 p-3">
                  <p className="text-slate-500 uppercase text-[10px] font-bold">Owner</p>
                  <p className="text-white truncate">{visibleTile.ownerId ? gameState.players.find(p => p.id === visibleTile.ownerId)?.username : 'Unowned'}</p>
                </div>
              </div>
              ...
              {visibleTile.type === 'special' && ![10, 30].includes(visibleTile.id) && (
                <p className="mt-3 text-sm text-cyan-100 bg-cyan-950/40 border border-cyan-500/30 rounded-lg p-3">
                  City event tile. Draw and resolve the event card for this space.
                </p>
              )}
              ...
              <div className="mt-4 flex gap-2">
                {!visibleTile.ownerId && visibleTile.price > 0 && isCurrentTurn && (
                  <button onClick={handleBuyProperty} className="flex-1 rounded-xl bg-emerald-600 hover:bg-emerald-500 px-4 py-3 text-white font-bold">
                    Buy
                  </button>
                )}
                {visibleTile.ownerId && visibleTile.ownerId !== activePlayer?.id && (
                  <button onClick={() => setSpaceModalOpen(false)} className="flex-1 rounded-xl bg-yellow-600 hover:bg-yellow-500 px-4 py-3 text-white font-bold">
                    Pay Rent
                  </button>
                )}
                <button
                  onClick={() => {
                    setSpaceModalOpen(false);
                    if (isCurrentTurn) onAction({ type: 'end_turn', payload: {}, timestamp: Date.now(), playerId });
                  }}
                  className="flex-1 rounded-xl bg-slate-700 hover:bg-slate-600 px-4 py-3 text-white font-bold"
                >
                  {visibleTile.price > 0 && !visibleTile.ownerId ? 'Skip' : 'Next Turn'}
                </button>
              </div>
            </div>
          </div>
      ...
      {/* Dice Display */}
      <div className="pointer-events-none absolute bottom-40 left-1/2 -translate-x-1/2 z-30">
        <div className={`px-5 py-3 rounded-2xl border shadow-2xl backdrop-blur transition-all ${isRolling ? 'scale-110 border-cyan-300 bg-cyan-950/95 shadow-cyan-400/40' : 'border-white/10 bg-slate-950/90'}`}>
          <p className="mb-2 text-center text-xs font-black uppercase tracking-[0.25em] text-cyan-200">
            {isRolling ? 'Rolling Dice' : isMoving ? 'Moving Token' : 'Last Roll'}
          </p>
          <div className="flex items-center gap-4">
            <div className={`w-16 h-16 rounded-2xl bg-white flex items-center justify-center text-4xl font-black text-slate-900 shadow-lg shadow-white/30 transition-transform ${isRolling ? 'animate-spin scale-110' : ''}`}>
              {displayDice.die1}
            </div>
            ...
            <span className="text-2xl text-white font-bold">=</span>
            <div className="w-20 h-20 rounded-2xl bg-gradient-to-br from-blue-500 to-cyan-500 flex items-center justify-center text-5xl font-black text-white shadow-lg shadow-cyan-500/40">
              {isRolling ? displayDice.die1 + displayDice.die2 : lastRoll}
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="absolute left-4 right-4 bottom-4 z-30">
        <div className="mx-auto max-w-7xl rounded-2xl border border-white/10 bg-slate-950/95 shadow-2xl backdrop-blur flex items-stretch gap-3 p-3">
          <div className="min-w-0 flex-1 max-w-sm rounded-xl border border-cyan-400/30 bg-slate-900/90 p-3 flex items-center gap-3">
            <div className="w-14 h-14 rounded-xl bg-gradient-to-br from-cyan-500 to-blue-700 flex items-center justify-center">
              <Home size={26} className="text-white" />
            </div>
            <div className="min-w-0">
              <p className="text-white font-bold truncate">{activePlayer?.username || 'Waiting'}</p>
              <p className="text-xs text-slate-400">Position: {(activePlayer?.position ?? 0) + 1} / {gameState.properties.length}</p>
              <p className="text-xs text-emerald-400 font-bold">Net Worth: {(activePlayer?.coins || 0).toLocaleString()} TC</p>
            </div>
          </div>

          <button
            onClick={handleRollDice}
            disabled={currentTurnUserId !== playerId || isSpectator || gameState.phase !== 'waiting_for_roll' || isRolling || isMoving || pendingTileAction || spaceModalOpen}
            className="px-8 rounded-xl bg-gradient-to-r from-blue-600 to-cyan-500 disabled:from-slate-700 disabled:to-slate-700 disabled:text-slate-400 text-white font-black flex items-center gap-2 justify-center min-w-[190px]"
          >
            <Dice5 size={24} />
            {isRolling ? 'ROLLING...' : isMoving ? 'MOVING...' : 'ROLL DICE'}
          </button>

          <div className="min-w-[170px] rounded-xl bg-slate-900/90 border border-white/10 p-3 flex flex-col justify-center">
            <p className="text-xs uppercase text-slate-500 font-bold">Last Roll</p>
            <p className="text-white text-2xl font-black">{displayDice.die1} + {displayDice.die2} = {isRolling ? displayDice.die1 + displayDice.die2 : lastRoll}</p>
          </div>

          <div className="flex items-stretch gap-2">
            {[
              { label: 'Trade', icon: Repeat2 },
              { label: 'Map', icon: MapPin },
              { label: 'Properties', icon: Landmark },
              { label: 'Menu', icon: Menu },
            ].map(({ label, icon: Icon }) => (
              <button key={label} className="w-24 rounded-xl bg-slate-900 hover:bg-slate-800 border border-white/10 text-white flex flex-col items-center justify-center gap-1">
                <Icon size={22} />
      ...
      <ModActionsPopup
        isOpen={!!modActionTarget}
        onClose={() => setModActionTarget(null)}
        targetUser={modActionTarget}
        targetUsername={modActionTarget?.username || ''}
        targetUserId={modActionTarget?.id || ''}
        streamId={streamId || gameState.streamId || gameState.matchId}
        hostId={gameState.players[0]?.id || ''}
      ...
      {/* Controls Help */}
      <div className="hidden absolute bottom-6 left-6 z-20">
