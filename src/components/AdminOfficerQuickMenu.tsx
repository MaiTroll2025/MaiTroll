...
const AdminOfficerQuickMenu: React.FC = () => {
  ...
  const isSecretary = profile.role === UserRole.SECRETARY || profile.role === UserRole.TROLL_CITY_SECRETARY || profile.role === UserRole.EXECUTIVE_SECRETARY;
  const isPastor = profile.is_pastor;
  const isAttorney = (profile as any).is_attorney;
  const isProsecutor = (profile as any).is_prosecutor;

  // Only show quick menu for regular Troll Officers (not lead, admin, secretary, pastor, attorney, prosecutor)
  if (!isOfficer || isLeadOfficer || isAdmin || isSecretary || isPastor || isAttorney || isProsecutor) return null;

  const getDashboardPath = () => {
    if (isAdmin) return '/admin';
    if (isLeadOfficer) return '/department-tools';
    if (isOfficer) return '/department-tools';
