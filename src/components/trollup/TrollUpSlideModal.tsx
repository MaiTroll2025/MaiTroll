...
export default function TrollUpSlideModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      console.error('[TrollUpModal] load error', e)
    }
  }, [user?.id])
  ...
  const handlePurchase = async (pkg: PackageRow, withAutoPay: boolean) => {
    if (!user?.id) {
    ...
    try {
      ...
      if (result?.success) {
        if (withAutoPay) {
          ...
          if (subError) console.error('[TrollUpModal] subscription error', subError)
        }
  ...
  const handleCancelSubscription = async (sub: SubscriptionRow) => {
    try {
  ...
  const myCoinBalance = Number(profile?.troll_coins ?? 0)
          ...
          <div className="p-5">
            ...
            <div className="mb-5 flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-lg font-black uppercase text-white">Troll Up</h2>
              </div>
              <button
                onClick={onClose}
                className="flex h-8 w-8 items-center justify-center rounded-lg border border-white/10 bg-white/5 text-slate-400"
              >
                <X className="h-4 w-4" />
              </button>
            </div>
            ...
            {/* Balance */}
            <div className="mb-5 rounded-xl border border-cyan-400/20 bg-slate-950/70 p-4 backdrop-blur-2xl">
            ...
            <section className="mb-6">
              <h3 className="mb-3 text-sm font-black uppercase tracking-tight text-white">Packages</h3>
              <div className="space-y-3">
                {packages.map((pkg) => {
                  ...
                  return (
                    <div key={pkg.id} className="rounded-xl border border-white/10 bg-white/[0.035] p-4">
                      <div className="mb-2 flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          <Flame className="h-4 w-4 text-orange-400" />
                          <h4 className="text-sm font-black uppercase text-white">{pkg.name}</h4>
                        </div>
            ...
            {/* Auto Pay */}
            <section className="mb-6">
              <h3 className="mb-3 text-sm font-black uppercase tracking-tight text-white">Auto Pay</h3>
              <div className="rounded-xl border border-white/10 bg-white/[0.035] p-4">
                <div className="mb-3 flex items-center gap-2">
                  <RefreshCw className="h-4 w-4 text-cyan-400" />
                  <h4 className="text-sm font-black uppercase text-white">Weekly Auto Pay</h4>
                </div>
            ...
            <section className="mb-6">
              <div className="rounded-xl border border-red-400/25 bg-red-500/10 p-4">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <h4 className="text-xs font-black uppercase text-red-300">No Refunds</h4>
            ...
            {/* My Campaigns */}
            <section className="mb-6">
              <h3 className="mb-3 text-sm font-black uppercase tracking-tight text-white">My Troll Up</h3>
              {campaigns.length === 0 ? (
                <p className="text-xs text-slate-500">No active campaigns yet. Slide up and get your bags! 💰</p>
              ) : (
                <div className="space-y-3">
                  {campaigns.map((camp) => {
                    ...
                    return (
                      <div key={camp.id} className="rounded-xl border border-white/10 bg-white/[0.035] p-4">
                        <div className="mb-2 flex items-center justify-between">
