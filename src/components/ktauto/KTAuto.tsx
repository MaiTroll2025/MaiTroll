...
export default function KTAuto() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (licenseData) setLicenseStatus(licenseData.license_status || 'none');
      } catch (err) {
  ...
  const handlePurchase = async () => {
    ...
    if (licenseStatus === 'suspended') {
      toast.error(`Cannot purchase vehicle: License is suspended. Complete the driver test and purchase insurance to restore your license.`);
      return;
    }

    if (licenseStatus === 'none') {
      toast.error("You need an active driver's license to purchase a vehicle. Please visit the DMV and take the driver test.");
      return;
    }
