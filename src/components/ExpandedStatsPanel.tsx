  ...
  useEffect(() => {
    ...
    const loadStats = async () => {
      try {
        ...
        // Load battle stats from profile if available
        ...
        if (battleProfileError) {
          console.warn('[ExpandedStatsPanel] Failed to load battle profile stats:', battleProfileError);
        }
        ...
        const battleLosses = typeof totalBattleMatches === 'number'
          ? Math.max(totalBattleMatches - battleWins, 0)
          : 0;
        const warTier = battleCrowns >= 50 ? 'Diamond' : battleCrowns >= 25 ? 'Platinum' : battleCrowns >= 10 ? 'Gold' : battleCrowns >= 5 ? 'Silver' : 'Bronze';

        const warStats = {
          warWins: battleWins,
          warLosses: battleLosses,
          warStreak: battleStreak,
          warTier,
        }
