...
export default function PromoAdCard({ ad, variant = 'sidebar', onClick }: PromoAdCardProps) {
  ...
  // Countdown timer
  useEffect(() => {
    if (!showLightbox || !ad.cta_link) return;
  ...
  const handleClick = async () => {
    ...
    // Start auto-redirect timer if CTA link exists
    if (ad.cta_link) {
      setCountdown(AUTO_REDIRECT_DELAY / 1000);
  ...
  return (
    ...
    >
      ...
      <div className={`relative z-10 h-full ${isHorizontal ? 'flex items-center justify-between p-3 md:p-4' : 'flex flex-col justify-end p-3'}`}>
        ...
        {showLightbox && lightboxVisible && ad.cta_link && countdown > 0 && (
          <div className="absolute top-2 left-2 z-20 flex items-center gap-2 px-3 py-1.5 rounded-full bg-purple-600/90 text-white text-xs font-medium">
            <span className="animate-pulse">↗</span>
            <span>Redirect in {countdown}s</span>
          </div>
        )}
      </div>
