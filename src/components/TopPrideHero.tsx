...
const ALL_PRIDE_CHALLENGES: Array<{ week: number; title: string; description: string; xp: string; color: string }> = [
  // Week 1
...
function PrideChallengesCard({ onOpenChallenges }: { onOpenChallenges: () => void }) {
  ...
  const [daysUntilSunday, setDaysUntilSunday] = useState(() => {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      const now = new Date()
      setCurrentWeek(Math.min(4, Math.max(1, Math.ceil(now.getDate() / 7))))
      const dayOfWeek = now.getDay()
      setDaysUntilSunday(dayOfWeek === 0 ? 0 : 7 - dayOfWeek)
    }, 3600000)
    return () => clearInterval(interval)
  }, [])

  const visibleChallenges = useMemo(
    () => ALL_PRIDE_CHALLENGES.filter(c => c.week <= currentWeek),
    [currentWeek]
  )
  const previewChallenges = useMemo(
    () => ALL_PRIDE_CHALLENGES.filter(c => c.week === currentWeek).slice(0, 3),
    [currentWeek]
  )
      ...
      <div className="relative z-10">
        <div className="flex items-center justify-between">
          ...
          <span className="rounded-full bg-gradient-to-r from-pink-500/20 to-purple-500/20 px-2.5 py-1 text-[10px] font-black text-pink-200">
            {visibleChallenges.length}/{ALL_PRIDE_CHALLENGES.length}
          </span>
        </div>
        ...
        <div className="mt-3 space-y-2">
          {previewChallenges.map((ch) => (
            <div key={ch.title} className={`rounded-xl border p-3 ${COLOR_MAP[ch.color] || 'border-white/10 bg-white/[0.04]'}`}>
              <div className="flex items-center justify-between gap-2">
                <p className="text-xs font-black text-white">{ch.title}</p>
                <span className={`text-[11px] font-black ${XP_COLOR_MAP[ch.color] || 'text-yellow-300'}`}>{ch.xp}</span>
              </div>
        ...
        {previewChallenges.length < ALL_PRIDE_CHALLENGES.filter(c => c.week === currentWeek).length && (
          <p className="mt-2 text-center text-[10px] text-slate-500">
            +{ALL_PRIDE_CHALLENGES.filter(c => c.week === currentWeek).length - 3} more this week
          </p>
        )}
