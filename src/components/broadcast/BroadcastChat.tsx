...
export default function BroadcastChat({
  ...
  // Realtime broadcast channel ref (singleton per streamId)
    ...
    const challengeChannel = supabase
      ...
      .on(
        ...
        (payload: any) => {
          ...
          if (import.meta.env.DEV) console.debug('[BroadcastChat] New challenge received:', challengeData);
  ...
  useEffect(() => {
    ...
    const moderationTimer = window.setInterval(fetchHostModerationState, 30000);
  ...
  useStreamRealtime(streamId, {
    onStream: (event) => {
      if (isHost) return;
      const newStatus = (event.new as any)?.status;
  ...
  const fetchedStreamIdsRef = useRef<Set<string>>(new Set())
  const fetchingStreamIdRef = useRef<string | null>(null)

  // Fetch initial messages (last 50) only when chat is opened.
  // Idempotent per streamId (safe under React StrictMode).
  useEffect(() => {
      if (import.meta.env.DEV) console.debug('[BroadcastChat] 🔄 Message fetch useEffect triggered, streamId:', streamId, 'isChatOpen:', isChatOpen)
      if (!streamId) return

      if (!isChatOpen) {
          if (import.meta.env.DEV) console.debug('[BroadcastChat] Chat is closed, skipping message fetch')
          return
      }

      if (fetchedStreamIdsRef.current.has(streamId)) {
          if (import.meta.env.DEV) console.debug('[BroadcastChat] Messages already fetched for streamId, skipping fetch:', streamId)
          return
      }

      if (fetchingStreamIdRef.current === streamId) {
          if (import.meta.env.DEV) console.debug('[BroadcastChat] Message fetch already in progress for streamId, skipping:', streamId)
          return
      }
...
const fetchMessages = async () => {
          ...
          if (data) {
              const processedMessages = data.reverse().map((m: any) => {
                  const uProfile = {
                      ...
                      rgb_username_expires_at:
                          m.user_rgb_expires_at || m.user_profiles?.rgb_username_expires_at,
                      glowing_username_color:
                          m.user_glowing_username_color || m.user_profiles?.glowing_username_color,
                  }
  ...
  useEffect(() => {
    ...
    // NOTE: we keep a dedicated singleton guard here instead of relying on React effect semantics.

    if (!broadcastChannelRef.current) {
      if (import.meta.env.DEV) console.debug('[BroadcastChat] 📡 realtime channel singleton: creating channel for:', channelName)
    } else {
      if (import.meta.env.DEV) console.debug('[BroadcastChat] 📡 realtime channel singleton: already have channel, current streamId:', subscribedStreamIdRef.current, 'incoming streamId:', streamId)
    }

    // If channel already exists for this streamId, do nothing.
    if (broadcastChannelRef.current && subscribedStreamIdRef.current === streamId) {
      if (import.meta.env.DEV) console.debug('[BroadcastChat] realtime channel already active, skipping setup. streamId:', streamId)
      return
    }

    // If streamId changed and we have an old channel, remove the exact instance.
    if (broadcastChannelRef.current && subscribedStreamIdRef.current !== streamId) {
      if (import.meta.env.DEV) console.debug('[BroadcastChat] realtime channel stream changed, cleaning up old channel. from:', subscribedStreamIdRef.current, 'to:', streamId)
      try {
    ...
    const broadcastChannel = supabase
        ...
        .on(
            ...
            (payload: any) => {
                ...
                if (import.meta.env.DEV) console.debug('[BroadcastChat] isGiftMessage:', isGiftMessage);
                
                if (msg.user_id === userIdRef.current && !isGiftMessage) {
                    return;
                }
                ...
                if (!isChatOpenRef.current) {
                    ...
                    if (import.meta.env.DEV) console.debug('[BroadcastChat] Message received while chat closed, incrementing unread');
        ...
        .on(
          ...
          (payload: any) => {
            ...
            if (import.meta.env.DEV) console.debug('[BroadcastChat] 🎁🎁🎁 Gift received for chat (gift_sent listener):', giftData);
        ...
        .subscribe((status) => {
          if (import.meta.env.DEV) console.debug('[BroadcastChat] 📡 Broadcast channel subscription status:', status);
        });

    broadcastChannelRef.current = broadcastChannel;

    // DB message fallback disabled for hot-path load. Chat uses optimistic UI,
  ...
  const sendMessage = async (e: React.FormEvent) => {
    ...
    if (import.meta.env.DEV) console.debug('💬 [BroadcastChat] sendMessage called', { 
      hasUser: !!user, 
      hasProfile: !!profile, 
    ...
    if (!input.trim()) {
        if (import.meta.env.DEV) console.debug('💬 [BroadcastChat] Empty input, ignoring');
        return;
    }
  ...
  const openModActionsForUser = (
    ...
    if (!targetUserId || !isOfficer) return;
  ...
  const deleteMessage = async (msgId: string) => {
      if (!streamId || !msgId) return;

      const { data, error } = await supabase.rpc('moderator_delete_stream_message', {
        p_stream_id: streamId,
        p_message_id: msgId,
      });

      if (error || !data?.success) {
        toast.error(error?.message || data?.message || 'Failed to delete message');
        return;
      }
            ...
            <div className="relative w-full">
                <input
                    ...
                    onClick={redirectGuestToAuth}
                     ...
                     readOnly={isGuest || streamEnded || hostChatDisabledByOfficer || userChatDisabled}
                     disabled={hostChatDisabledByOfficer || streamEnded || userChatDisabled}
