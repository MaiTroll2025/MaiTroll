...
export default function TeacherCoursePage() {
  ...
  const [form, setForm] = useState({
    ...
    difficulty_level: 'beginner', max_students: 20, enrollment_fee: 1000,
        ...
        if (isEdit && courseId) {
          ...
          if (course && mounted) {
            setForm({
              ...
              enrollment_fee: course.enrollment_fee || 1000, currency_type: course.currency_type || 'troll_coins',
              ...
              timezone: course.timezone || 'America/New_York', status: course.status || 'draft',
            });
          }
        }
  ...
  const toggleOER = (resource: OERResource) => {
    setSelectedOER(prev => prev.find(r => r.id === resource.id) ? prev.filter(r => r.id !== resource.id) : [...prev, resource]);
  };
