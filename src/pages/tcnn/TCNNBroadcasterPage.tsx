 ...
 * Handles both fresh start and redirect from SetupPage with existing streamId.
 */
...
import TCNNVirtualStudio, {
  TCNNVirtualStudioHandle,
  BackgroundMode,
} from '@/components/tcnn/TCNNVirtualStudio';
...
export default function TCNNBroadcasterPage() {
  ...
  const handleStudioReady = useCallback(() => {
    console.log('[TCNNBroadcaster] Studio ready');
    setStudioLoading(false);
  }, []);

  const handleStudioError = useCallback((error: string) => {
    console.error('[TCNNBroadcaster] Studio error:', error);
    setStudioLoading(false);
    toast.error('Studio error: ' + error);
  }, []);

  const handleStudioModeChange = useCallback((mode: 'full' | 'simple') => {
    console.log('[TCNNBroadcaster] Studio mode changed:', mode);
    setStudioMode(mode);
  }, []);
  ...
  const ensureStudioRunning = useCallback(async () => {
    if (!studioRef.current) {
      throw new Error('TCNN studio is not ready yet');
    }

    if (!studioRef.current.isRunning()) {
      console.log('[TCNNBroadcaster] Starting studio...');
      await studioRef.current.start();
    }
  }, []);

  const publishStudioTracks = useCallback(async (room: Room) => {
    ...
    if (compositeStream) {
      ...
      if (videoTracks.length > 0) {
        ...
        console.warn('[TCNNBroadcaster] No composite video track found');
      }
    } else {
      console.warn('[TCNNBroadcaster] No composite studio stream found');
    }
    ...
    try {
      ...
      toast.warning('Mic could not be published, but video is live');
    }
  }, [ensureStudioRunning]);
  ...
  const connectLiveKitRoom = useCallback(
    async (targetStreamId: string) => {
      ...
      await publishStudioTracks(room);

      return room;
    },
    [user, publishStudioTracks]
  );

  const connectToExistingStream = useCallback(
    async (targetStreamId: string) => {
      ...
      try {
        const { data: existingStream, error: fetchError } = await supabase
          .from('streams')
          .select('id, user_id, broadcaster_id, agora_channel, is_live, status, title')
        ...
        const ownsStream =
          existingStream.user_id === user.id ||
          existingStream.broadcaster_id === user.id;
        ...
        await connectLiveKitRoom(targetStreamId);

        if (!existingStream.is_live || existingStream.status !== 'live') {
          await supabase
            .from('streams')
            .update({
              status: 'live',
    ...
    [user, connectLiveKitRoom, setupRealtimeChannel]
  );

  useEffect(() => {
    const startStudioOnMount = async () => {
      if (!user || isLive) return;

      try {
        await ensureStudioRunning();
      } catch (error) {
        console.warn('[TCNNBroadcaster] Studio mount start failed:', error);
      }
    };

    startStudioOnMount();
  }, [user, isLive, ensureStudioRunning]);

  useEffect(() => {
    if (paramStreamId && user && !isLive && !autoConnectAttempted.current && !studioLoading) {
      connectToExistingStream(paramStreamId);
    }
  }, [paramStreamId, user, isLive, studioLoading, connectToExistingStream]);
  ...
  useEffect(() => {
    ...
    const startMetering = async () => {
      try {
        ...
        console.warn('[TCNNBroadcaster] Audio metering unavailable');
      }
    };
  ...
  const handleGoLive = useCallback(async () => {
    ...
    try {
      ...
      await connectLiveKitRoom(newStreamId);
  ...
  }, [user, profile, isStarting, connectLiveKitRoom, setupRealtimeChannel]);
  ...
  const handleEndBroadcast = useCallback(async () => {
    try {
      ...
      console.error('[TCNNBroadcaster] End broadcast error:', err);
  ...
  if (!isNewsCaster && !isChiefNewsCaster && !profile?.is_admin && profile?.role !== 'admin') {
    return null;
  }
    ...
    <div className="h-screen w-full bg-[#060a14] text-white flex overflow-y-auto overflow-x-hidden md:overflow-hidden select-none">
      <aside className="w-[260px] flex-shrink-0 bg-[#0a0f1e]/95 border-r border-white/[0.06] flex flex-col backdrop-blur-xl">
        ...
        <div className="p-4 border-b border-white/[0.06]">
          ...
          >
            ...
            <div>
              ...
              {isLive && <p className="text-[10px] text-white/40">{viewerCount} viewers</p>}
            </div>
          </div>
        </div>
        ...
        <div className="p-4 border-b border-white/[0.06] space-y-3">
          ...
          {!isLive ? (
            <button
              ...
              disabled={isStarting || isAutoConnecting || studioLoading}
            ...
            >
              {isStarting || isAutoConnecting ? (
                <Loader2 className="w-4 h-4 animate-spin" />
              ) : (
                <Play className="w-4 h-4" fill="currentColor" />
              )}

              {isStarting || isAutoConnecting ? 'Starting...' : 'Go Live'}
            </button>
          ) : (
            <button
              onClick={handleEndBroadcast}
              disabled={!isLive}
              className="w-full flex items-center justify-center gap-2 px-4 py-3 rounded-xl bg-white/[0.06] hover:bg-red-500/20 border border-white/[0.08] hover:border-red-500/30 text-white/80 hover:text-red-400 font-medium text-sm transition-all disabled:opacity-40"
            >
                ...
                <div className="h-2 bg-white/[0.06] rounded-full overflow-hidden">
                  <div
                    ...
                    style={{
                      width: `${micLevel}%`,
                      background:
                        micLevel > 80 ? '#ef4444' : micLevel > 50 ? '#f59e0b' : '#06b6d4',
                    }}
                  />
                </div>
              </div>
        ...
        <div className="p-4 flex-1">
          ...
          <button
            onClick={() => setShowLowerThirdEditor((prev) => !prev)}
          ...
          >
            ...
            <ChevronDown
              className={`w-3.5 h-3.5 transition-transform ${
                showLowerThirdEditor ? 'rotate-180' : ''
              }`}
            />
          </button>
        ...
        <div className="p-4 border-t border-white/[0.06]">
          <p className="text-[10px] text-white/20 text-center">
            FPS: {fps} | {backgroundMode} | {studioMode}
          </p>
        </div>
      </aside>
        ...
        <div className="flex-1 flex items-center justify-center p-4">
          <div className="relative w-full h-full max-w-[1280px] max-h-[720px] aspect-video rounded-2xl overflow-hidden shadow-2xl shadow-black/50 border border-white/[0.06]">
            <TCNNVirtualStudio
              ...
              onFpsUpdate={setFps}
              showLoadingOverlay={studioLoading || isAutoConnecting}
            ...
            {isAutoConnecting && (
              <div className="absolute inset-0 z-40 bg-black/70 backdrop-blur-sm flex items-center justify-center">
                <div className="text-center">
                  ...
                  <p className="text-sm text-white/70">Connecting to TCNN Studio...</p>
                  <p className="text-xs text-white/30 mt-1">Joining existing stream</p>
                </div>
              </div>
            )}
      ...
      <aside
        className={`w-[300px] flex-shrink-0 bg-[#0a0f1e]/95 border-l border-white/[0.06] flex flex-col backdrop-blur-xl transition-all duration-300 ${
          chatOpen ? '' : 'w-0 overflow-hidden border-0'
        }`}
      >
          ...
          {messages.length === 0 && (
            <div className="text-center py-12">
              ...
              <p className="text-xs text-white/30">
                Chat will appear here when viewers send messages
              </p>
            </div>
          )}
