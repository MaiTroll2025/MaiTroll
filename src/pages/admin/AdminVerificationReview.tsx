...
export default function AdminVerificationReview() {
  ...
  const isAdmin = profile?.role === 'admin' || profile?.role === 'secretary' || profile?.is_admin || (profile as any)?.is_secretary
  ...
  useEffect(() => {
    ...
    const interval = window.setInterval(loadRequests, 60_000)
