...
export default function TempAdminDashboard() {
  ...
  const [arrestBail, setArrestBail] = useState(100)
  ...
  // Load online users with realtime updates
  useEffect(() => {
    const loadOnlineUsers = async () => {
      ...
      try {
        ...
        const { data } = await supabase
          .from('online_users')
          .select('*, user_profiles(id, username, avatar_url, role, troll_role, is_banned)')
        ...
        if (data) {
          ...
          const flattened = data.map(u => ({
            id: u.user_id,
            username: u.user_profiles?.username || 'Unknown',
            avatar_url: u.user_profiles?.avatar_url,
            role: u.user_profiles?.role,
            troll_role: u.user_profiles?.troll_role,
            is_banned: u.user_profiles?.is_banned,
  ...
  // Handle arrest user
  const handleArrestUser = async () => {
    if (!selectedUser || !arrestReason) return toast.error('User and reason required')
    
    try {
      const { error } = await supabase.from('jail').insert({
        user_id: selectedUser.id,
        reason: arrestReason,
        severity: arrestSeverity,
        bond_amount: arrestBail,
      ...
      toast.success(`${selectedUser.username} has been arrested!`)
              ...
              <div className="bg-zinc-900/50 border border-zinc-800 rounded-xl p-6">
                ...
                <div className="relative mb-4">
                  ...
                  <input
                    ...
                    className="w-full bg-black/40 border border-zinc-700 rounded-lg pl-10 pr-4 py-2"
                  />
                </div>
                  ...
                  {onlineUsers
                    ...
                    .map(user => (
                      ...
                      >
                        ...
                        {user.is_banned && (
                          <span className="text-xs bg-red-600 text-white px-2 py-0.5 rounded">Banned</span>
                        )}
                      </button>
                    ))}
                </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="space-y-4">
                      <h4 className="font-bold text-orange-400 flex items-center gap-2">
                        <UserMinus className="w-4 h-4" /> Arrest User
                      </h4>
                      <div>
                        ...
                        <input
                          ...
                          className="w-full bg-black/40 border border-zinc-700 rounded p-2"
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        <div>
                          ...
                          <select
                            value={arrestSeverity}
                            onChange={e => setArrestSeverity(e.target.value)}
                            className="w-full bg-black/40 border border-zinc-700 rounded p-2"
                          >
                        ...
                        <div>
                          <label className="block text-xs text-gray-400 mb-1">Bail (coins)</label>
                          <input
                            type="number"
                            value={arrestBail}
                            onChange={e => setArrestBail(Number(e.target.value))}
                            className="w-full bg-black/40 border border-zinc-700 rounded p-2"
                          />
                        </div>
                      </div>
                      <button
                        onClick={handleArrestUser}
                        disabled={!arrestReason}
                        className="w-full bg-orange-600 hover:bg-orange-500 disabled:bg-zinc-700 text-white font-bold py-2 px-4 rounded-lg flex items-center justify-center gap-2"
                      >
                    ...
                    {/* Quick Actions */}
                    <div className="space-y-3">
                      <h4 className="font-bold text-blue-400">Quick Actions</h4>
                      <button
                        onClick={() => {
                      ...
                      >
                        <UserMinus className="w-4 h-4" /> Ban User
                      </button>
                      <button
                        onClick={() => {
                      ...
                      >
                        <UserMinus className="w-4 h-4" /> Kick from Streams
                      </button>
                      <button
                        onClick={() => setSelectedUser(null)}
                        className="w-full bg-zinc-700 hover:bg-zinc-600 text-white font-bold py-2 px-4 rounded-lg"
                      >
                        Cancel
                      </button>
                    </div>
