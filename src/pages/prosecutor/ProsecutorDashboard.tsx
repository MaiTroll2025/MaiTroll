...
export default function ProsecutorDashboard() {
  ...
  const fetchProsecutorData = async () => {
    try {
      ...
      // Get cases where the prosecutor is assigned (active cases for the prosecutor)
      ...
      // Get cases that are available for a prosecutor to take (no prosecutor assigned and status allows)
      ...
      // Fetch profiles for plaintiff and defendant for active cases
      ...
      if (activeUserIds.length > 0) {
        const { data: profiles } = await supabase
          ...
          .in('id', activeUserIds);
        if (profiles) {
          profiles.forEach(p => { activeUserMap[p.id] = p; });
        }
      }

      // Fetch profiles for plaintiff and defendant for pending cases
      const pendingUserIds = [...new Set(
        (pendingData || []).flatMap(c => [c.plaintiff_id, c.defendant_id]).filter(Boolean)
      )];
      const pendingUserMap: Record<string, any> = {};
      ...
      const transformedActive = (activeData || []).map((c: any) => ({
        ...
        plaintiff: activeUserMap[c.plaintiff_id] || null,
        defendant: activeUserMap[c.defendant_id] || null,
  ...
  return (
    <div className="bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 text-white p-6">
