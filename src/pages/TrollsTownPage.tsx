...
const HOME_TIERS = [
  ...
  },
  {
    ...
    name: 'Urban Loft',
    price: 250_000,
    description: 'Stylish loft in a busy part of Mai Troll.',
  ...
  },
  {
    ...
    name: 'Troll Mansion',
    price: 50_000_000,
    description: 'A luxurious mansion for the elite of Mai Troll.',
...
const TrollsTownPage: React.FC = () => {
      ...
      try {
        ...
        if (properties && properties.length > 0) {
          ...
          if (active) {
            ...
            // Ensure user profile reflects the active property for neighborhood onboarding
            try {
              ...
              if (!currentProfile?.house_id || currentProfile.house_id !== active.id) {
                await supabase
                  .from('user_profiles')
                  .update({ house_id: active.id })
                  .eq('id', user.id)
              }
            } catch (profileErr) {
              console.error('Failed to sync active property with user profile:', profileErr)
            }
          } else {
  ...
  const handleSellHomeToBank = async () => {
    ...
    try {
      ...
      if (properties && properties.length > 0) {
        ...
        // Clear house_id from user profile if no properties left
        try {
          await supabase
            .from('user_profiles')
          ...
          console.error('Failed to clear house_id from user profile:', profileErr)
        }
      }
  ...
  const handleBuyTieredHome = async (tierId: string) => {
    ...
    try {
      ...
      if (newProperty) {
        try {
          localStorage.setItem(`MaiTroll_home_owned_${user.id}`, 'true')
        } catch {}
      }
  ...
  const handleBuyProperty = async (property: PropertyRow) => {
    ...
    try {
      ...
      // Update user profile to reflect property ownership for neighborhood onboarding
      try {
        await supabase
          .from('user_profiles')
          .update({ house_id: property.id })
          .eq('id', user.id)
      } catch (profileErr) {
        console.error('Failed to update user profile after property purchase:', profileErr)
        // Don't block the purchase if profile update fails
      }
  ...
  const handleSetActiveHome = async () => {
    ...
    try {
      ...
      // Update user profile to reflect active property for neighborhood onboarding
      try {
        await supabase
          .from('user_profiles')
          .update({ house_id: myProperty.id })
          .eq('id', user.id)
      } catch (profileErr) {
        console.error('Failed to update user profile after setting active home:', profileErr)
      }
  ...
  if (!user || !profile) {
    return (
      <div className={`min-h-screen ${MaiTrollTheme.backgrounds.primary} text-white flex items-center justify-center`}>
        <div className={`${MaiTrollTheme.backgrounds.card} ${MaiTrollTheme.borders.glass} border rounded-xl p-8 text-center max-w-md mx-auto`}>
          <h1 className="text-2xl font-bold mb-3">Troll Town</h1>
          <p className={`${MaiTrollTheme.text.muted} mb-4`}>
            You need to be logged in to manage your home in Troll Town.
          </p>
          <button
            onClick={() => navigate('/auth')}
            className={`px-6 py-3 rounded-lg ${MaiTrollTheme.gradients.button} hover:opacity-90 font-semibold`}
          >
            Log In
          </button>
        </div>
  ...
  return (
    <div className={`min-h-screen ${MaiTrollTheme.backgrounds.primary} text-white p-6`}>
      <div className="max-w-6xl mx-auto space-y-6">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div className="flex items-center gap-3">
            <div className={`w-12 h-12 rounded-xl ${MaiTrollTheme.gradients.button} flex items-center justify-center`}>
          ...
          <div className="flex flex-col items-end gap-1">
            <span className={`text-xs uppercase tracking-widest ${MaiTrollTheme.text.muted}`}>TrollCoins</span>
      ...
      {/* Navigation Tabs */}
      <div className={`flex gap-4 mb-6 border-b ${MaiTrollTheme.borders.glass}`}>
        <button
          onClick={() => setActiveTab('real_estate')}
          className={`pb-3 text-sm font-medium transition-colors relative ${
            activeTab === 'real_estate' ? 'text-emerald-400' : `${MaiTrollTheme.text.muted} hover:text-white`
          }`}
        >
          Real Estate
        ...
        <button
          ...
          className={`pb-3 text-sm font-medium transition-colors relative ${
            activeTab === 'ktauto' ? 'text-blue-400' : `${MaiTrollTheme.text.muted} hover:text-white`
          }`}
        >
          KTAuto Dealership
          {activeTab === 'ktauto' && (
        ...
        <button
          ...
          className={`pb-3 text-sm font-medium transition-colors relative ${
            activeTab === 'my_garage' ? 'text-purple-400' : `${MaiTrollTheme.text.muted} hover:text-white`
          }`}
        >
          My Garage
          {activeTab === 'my_garage' && (
      ...
      <div className={`grid grid-cols-1 xl:grid-cols-3 gap-6 ${activeTab === 'ktauto' || activeTab === 'my_garage' ? 'hidden' : ''}`}>
        <div className="xl:col-span-2 space-y-6">
          <div className={`${MaiTrollTheme.backgrounds.card} border ${MaiTrollTheme.borders.glass} rounded-2xl p-6`}>
            ...
            <div className="bg-black/40 border border-purple-500/30 rounded-2xl p-6">
              ...
              {loading ? (
                ...
                <div className="space-y-5">
                  ...
                  <div className={`border ${MaiTrollTheme.borders.glass} rounded-xl p-4 mt-4 flex flex-col md:flex-row md:items-center md:justify-between gap-3`}>
                    <div>
                      <p className={`text-xs ${MaiTrollTheme.text.muted} uppercase tracking-widest`}>
                        Sell Home To Bank
                      </p>
                      <p className={`text-[11px] ${MaiTrollTheme.text.muted}`}>
                  ...
                  <div className={`border ${MaiTrollTheme.borders.glass} rounded-xl p-4 space-y-4`}>
                    <div className="flex items-center justify-between">
                      <div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="space-y-3">
                        <p className={`text-[11px] ${MaiTrollTheme.text.muted} uppercase tracking-widest`}>
                          Available Upgrades
                        </p>
                        ...
                        {availableUpgrades.map(up => (
                          <div
                            key={up.id}
                            className={`border ${MaiTrollTheme.borders.glass} rounded-lg p-3 flex flex-col gap-2 ${MaiTrollTheme.backgrounds.card}`}
                          >
                            <div className="flex items-center justify-between">
                              <div>
                                <p className="text-sm font-semibold">{up.name}</p>
                                <p className={`text-[11px] ${MaiTrollTheme.text.muted}`}>{up.description}</p>
                              </div>
                              <div className="text-right">
                                <p className={`text-xs ${MaiTrollTheme.text.muted}`}>{up.category}</p>
                            ...
                            <button
                              ...
                              className={`mt-1 px-3 py-1.5 rounded-lg ${MaiTrollTheme.components.buttonPrimary} text-xs font-semibold disabled:opacity-60 text-white`}
                            >
                    ...
                    {upgrades.length === 0 && (
                        ...
                        {upgrades.map(up => {
                          ...
                          return (
                            <div
                              key={up.id}
                              className={`border ${MaiTrollTheme.borders.glass} rounded-lg p-3 ${MaiTrollTheme.backgrounds.card} space-y-2`}
                            >
                              <div className="flex items-center justify-between">
                                <div>
          ...
          <div className="space-y-4">
            <div className={`${MaiTrollTheme.backgrounds.card} border ${MaiTrollTheme.borders.glass} rounded-2xl p-5`}>
                ...
                <div className="space-y-3 max-h-[420px] overflow-y-auto pr-1">
                  {listings.map(row => {
                    ...
                    return (
                      <div
                        key={row.id}
                        className={`border ${MaiTrollTheme.borders.glass} rounded-lg p-3 ${MaiTrollTheme.backgrounds.card} space-y-2 ${!isAvailable ? 'opacity-50' : ''}`}
                      >
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-3">
                            <div className={`w-20 h-24 rounded-lg overflow-hidden border ${MaiTrollTheme.borders.glass}`}>
                              <img
                          ...
                          <div className="text-right">
                            {isAvailable ? (
                              <>
                                <p className={`text-xs ${MaiTrollTheme.text.muted}`}>Ask Price</p>
                                <p className="text-sm font-semibold text-yellow-300">
                                  {(row.ask_price || 0).toLocaleString()} TC
                                </p>
                                <p className={`text-[10px] ${MaiTrollTheme.text.muted}`}>
                        ...
                        <div className="flex items-center justify-between">
                          <div>
                            <p className={`text-xs ${MaiTrollTheme.text.muted}`}>System Value</p>
            ...
            <div className={`${MaiTrollTheme.backgrounds.card} border ${MaiTrollTheme.borders.glass} rounded-2xl p-4 text-xs ${MaiTrollTheme.text.muted} space-y-2`}>
            ...
            <div className={`${MaiTrollTheme.backgrounds.card} border ${MaiTrollTheme.borders.glass} rounded-2xl p-4 text-xs ${MaiTrollTheme.text.secondary} space-y-3`}>
              <div className="flex items-center justify-between mb-1">
                <div className="flex items-center gap-2">
              ...
              {loadingTransactions ? (
                <div className={`${MaiTrollTheme.text.muted} text-xs`}>Loading transactions...</div>
              ) : transactions.length === 0 ? (
                <div className={`${MaiTrollTheme.text.muted} text-xs`}>
                ...
                <div className="space-y-2 max-h-56 overflow-y-auto pr-1">
                  {transactions.map(tx => {
                    ...
                    return (
                      <div
                        key={tx.id}
                        className={`flex items-center justify-between border ${MaiTrollTheme.borders.glass} rounded-lg px-2 py-1.5 ${MaiTrollTheme.backgrounds.card}`}
                      >
                        <div className="flex flex-col">
                          <span className="text-[11px] font-semibold text-white">
                            {label}
                          </span>
                          <span className={`text-[10px] ${MaiTrollTheme.text.muted}`}>
                        ...
                        <div className="text-right">
                          ...
                          {typeof tx.balance_after === 'number' && (
                            <div className={`text-[10px] ${MaiTrollTheme.text.muted}`}>
