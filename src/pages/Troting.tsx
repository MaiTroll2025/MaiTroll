           ...
           setPitches(current =>
             current.map(p =>
               p.id === payload.new.id
                 ? {
                     ...p,
                     vote_count: payload.new.vote_count,
                     up_votes: payload.new.up_votes,
                     down_votes: payload.new.down_votes,
                   }
                 : p
             )
           )
...
const handleVote = async (pitch: Pitch, direction: 'up' | 'down') => {
     ...
     try {
       ...
       if (data.success) {
         ...
         // Update local state with returned vote counts
         setPitches(current =>
           current.map(p =>
             p.id === pitch.id
               ? {
                   ...p,
                   vote_count: data.vote_count ?? p.vote_count,
                   up_votes: data.up_votes ?? p.up_votes,
                   down_votes: data.down_votes ?? p.down_votes,
                 }
               : p
           )
         )
