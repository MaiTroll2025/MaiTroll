    ...
    if (useCredit) {
       ...
       if (creditError) {
         ...
         if (creditSuccess === true) {
            ...
            // Refresh profile so credit_used updates in the UI
            try {
              const { refreshProfile } = useAuthStore.getState()
              await refreshProfile()
            } catch (e) {
              console.warn('[deductCoins] profile refresh after credit spend failed', e)
            }
    ...
    if (coinType === 'troll_coins' || !coinType) {
      ...
      try {
        ...
        if (profile && profile.id === userId) {
          ...
          if (!(metadataPayload as any)?.skipProfileRefresh) {
      ...
      if (type === 'gift_sent') {
        sendNotification(
          ...
          'coin_gifted',
          'Coins Gifted',
          `You gifted ${normalizedAmount} coins!`,
          { ...metadata, amount: normalizedAmount, type }
        )
      }

      // Global event for large gifts
      if (type === 'gift_sent' && normalizedAmount > 1000) {
        void supabase.rpc('create_global_event', {
            p_type: 'gift',
            p_title: `Someone just gifted ${normalizedAmount} coins!`,
            p_icon: 'gift',
            p_metadata: { ...metadata, amount: normalizedAmount, type }
        }).then(({ error }) => {
...
export async function addCoins(params: {
  ...
  try {
    ...
    if (type === 'gift_received' && amount > 1000) {
        void supabase.rpc('create_global_event', {
            p_type: 'gift',
            p_title: `Someone just received a gift of ${amount} coins!`,
            p_icon: 'gift',
            p_metadata: { ...metadata, amount, type }
        }).then(({ error }) => {
          if (error) console.error('Failed to announce gift event:', error)
        })
    }
