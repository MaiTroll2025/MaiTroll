...
function ActiveElectionsSection() {
  ...
  useEffect(() => {
    ...
    const fetchElections = async () => {
      try {
        // Fetch recent elections from government_elections table
        const { data: electionData } = await supabase
          .from('government_elections')
          .select('title, status, candidate_count, vote_count')
          .order('created_at', { ascending: false })
          .limit(5)

        if (electionData && electionData.length > 0 && mounted) {
          const formatted = electionData.map(e => ({
            position: e.title || 'General Election',
            candidates: e.candidate_count || 0,
            votes: e.vote_count 
              ? (e.vote_count >= 1000 ? (e.vote_count / 1000).toFixed(0) + 'K+' : e.vote_count.toString())
              : '0',
          ...
          // Fallback if no data
          setElections([
            { position: 'No active elections', candidates: 0, votes: '0', status: 'Check back later' },
          ])
        }
        setLoading(false)
      } catch (error) {
    ...
    <div className="space-y-4">
      {loading ? (
        Array.from({ length: 3 }).map((_, idx) => (
          <div key={idx} className="flex items-center justify-between p-4 bg-slate-900/30 border border-slate-800 rounded-xl animate-pulse">
            <div>
...
interface GovernmentStats {
  totalVoters: number;
  electedOfficials: number;
  courtCases: number;
}

function GovernmentStatsSection() {
  const [stats, setStats] = useState<GovernmentStats>({
    totalVoters: 0,
    electedOfficials: 0,
    courtCases: 0,
  })
  ...
  useEffect(() => {
    ...
    const fetchStats = async () => {
      try {
        // Total users as voters proxy
        const { count: totalVoters } = await supabase
          .from('user_profiles')
          .select('*', { count: 'exact', head: true })

        // Elected officials from government_positions
        const { data: officialsData } = await supabase
          .from('government_positions')
          .select('id')
          .eq('status', 'filled')

        // Court cases from court_cases
        const { count: courtCases } = await supabase
          .from('court_cases')
          .select('*', { count: 'exact', head: true })
        ...
        if (mounted) {
          setStats({
            ...
            electedOfficials: officialsData?.length || 0,
  ...
  const statItems = [
    { value: loading ? '...' : formatNumber(stats.totalVoters), label: 'Total Voters' },
    { value: loading ? '...' : stats.electedOfficials.toString(), label: 'Elected Officials' },
    { value: loading ? '...' : formatNumber(stats.courtCases), label: 'Court Cases' },
  ]
...
const trendingSearches = [
  ...
  { term: 'becoming an officer', category: 'Career' },
]
    ...
    >
      ...
      <CTASection
        ...
        primaryAction={{ label: 'View Elections', path: '/government' }}
         secondaryAction={{ label: 'Apply for Office', path: '/apply' }}
      />
    </SEOLayout>
  )
}