...
export const spendCoins = async (
  ...
  try {
    ...
    const { data, error } = await supabase.rpc('troll_bank_spend_coins_secure', {
      p_user_id: userId,
      p_amount: amount,
      p_bucket: transaction.type === 'gift' ? 'gifted' : 'paid',
...
export const earnCoins = async (
  ...
  try {
    // Use database transaction for atomicity
    // earn_coins RPC created in supabase/migrations migration to alias troll_bank_credit_coins
