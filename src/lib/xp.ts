...
const loggedGiftXpErrors = new Set<string>();

/**
 * Process Gift XP Logic (Server-Side)
 * Current DB RPC accepts direct XP award inputs: p_user_id, p_amount.
 * Gift completion must not fail if the XP RPC is unavailable.
 */
export async function processGiftXp(userId: string, amount: number) {
  if (!userId || !Number.isFinite(amount) || amount <= 0) {
    console.error('[processGiftXp] Missing userId or amount');
  ...
  if (error) {
    const key = `${userId}:${Math.floor(amount)}:${error.code || error.message}`;
    if (!loggedGiftXpErrors.has(key)) {
      loggedGiftXpErrors.add(key);
      console.warn('[processGiftXp] Error calling RPC:', error);
    }
