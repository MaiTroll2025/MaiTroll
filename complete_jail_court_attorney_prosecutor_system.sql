...
BEGIN
    SELECT username INTO v_arrestor_name FROM public.user_profiles WHERE id = p_arrested_by;
    ...
    PERFORM create_jail_notification(
        ...
        jsonb_build_object('days', p_days, 'reason', p_reason, 'arrested_by', p_arrested_by)
    );
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
BEGIN
    ...
    PERFORM notify_arrest(p_user_id, p_sentence_days, p_reason, p_arrested_by);
