-- ============================================================
-- FORCE FIX: Profile Frame System Tables
-- This drops and recreates the tables with correct column types
-- Run this in Supabase SQL Editor
-- ============================================================

-- 1. Drop old/unwanted tables
DROP TABLE IF EXISTS public.profile_frame_tiers CASCADE;

-- 2. Drop and recreate profile_frames with text id
DROP TABLE IF EXISTS public.profile_frames CASCADE;
...
-- 3. Drop and recreate user_profile_frames with text frame_id
DROP TABLE IF EXISTS public.user_profile_frames CASCADE;
...
-- 4. Create indexes
CREATE INDEX idx_user_profile_frames_user_id ON public.user_profile_frames(user_id);
CREATE INDEX idx_user_profile_frames_equipped ON public.user_profile_frames(user_id, is_equipped) WHERE is_equipped = true;
CREATE INDEX idx_profile_frames_rarity ON public.profile_frames(rarity);
CREATE INDEX idx_profile_frames_active ON public.profile_frames(is_active) WHERE is_active = true;
...
DROP POLICY IF EXISTS "Frames are publicly readable" ON public.profile_frames;
DROP POLICY IF EXISTS "Users can read own frames" ON public.user_profile_frames;
DROP POLICY IF EXISTS "Users can insert own frames" ON public.user_profile_frames;
DROP POLICY IF EXISTS "Users can update own frames" ON public.user_profile_frames;
DROP POLICY IF EXISTS "Users can delete own frames" ON public.user_profile_frames;
...
BEGIN
  ...
  IF col_type = 'text' THEN
    RAISE NOTICE 'SUCCESS: user_profile_frames.frame_id is text type';
  ELSE
    RAISE WARNING 'FAILURE: user_profile_frames.frame_id is still % type', col_type;
  END IF;
END $$;
