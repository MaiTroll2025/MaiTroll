...
// Define pages and their tables
const pageTables = {
  'auth': ['signup_queue', 'user_event_dismissals', 'admin_password_resets', 'critical_alerts', 'jail_ip_violations', 'jail_security_violations', 'daily_login_posts', 'active_sessions', 'concurrent_login_details'],
  'home': ['page_visibility', 'sidebar_updates', 'user_sidebar_views', 'support_screen_sessions', 'city_events', 'global_events', 'broadcast_rankings', 'featured_broadcasts', 'admin_broadcasts', 'city_ads', 'user_advertisements'],
  'profile': ['user_profile_badges', 'user_profile_roles', 'profile_frames', 'profile_tab_visibility', 'user_active_items', 'user_avatar_customization', 'user_broadcast_theme_purchases', 'user_broadcast_theme_state', 'user_entrance_audio', 'user_entrance_effects', 'user_profile_frames', 'user_perks', 'user_badges', 'user_stats', 'user_levels', 'user_subscriptions'],
  ...
  'notifications': ['notifications', 'user_follows', 'user_followers', 'user_reports', 'moderation_actions', 'moderation_reports', 'user_blocks', 'user_bans', 'user_mutes', 'chat_blocks', 'support_tickets', 'support_goal_reminder_dismissals', 'support_screen_sessions', 'app_bug_reports', 'beta_feedback', 'beta_feedback_replies', 'beta_feedback_audit_log', 'beta_feedback_internal_notes', 'bug_alerts', 'system_alerts', 'emergency_alerts', 'scheduled_announcements'],
  'coins': ['coin_ledger', 'coin_orders', 'manual_coin_orders', 'coin_audit_log', 'stripe_customers', 'saved_cards', 'payment_methods', 'payout_batches', 'payout_runs', 'payouts', 'paypal_transactions', 'cashout_tiers', 'cashout_requests', 'user_credit', 'credit_events', 'fast_pay_applications'],
  ...
  'treasury': ['troll_city_treasury', 'treasury_transactions', 'treasury_role_allocations', 'treasury_payout_runs', 'treasury_payout_items', 'payout_batches', 'payout_runs', 'payouts', 'platform_profit', 'platform_revenue', 'admin_pool', 'admin_pool_ledger', 'admin_pool_transactions'],
  ...
  'utromail': ['utromail_accounts', 'utromail_threads', 'utromail_thread_members', 'utromail_messages', 'utromail_read_status', 'utromail_attachments', 'utromail_blocks', 'utromail_requests', 'utromail_reports', 'utromail_notifications'],
  ...
  'government': ['government_laws', 'government_history', 'government_positions', 'government_elections', 'president_proposals', 'president_audit_logs', 'president_candidates', 'president_elections', 'president_votes', 'president_appointments', 'president_treasury_balance', 'law_votes', 'protests', 'protest_participants', 'emergency_powers_log', 'city_reputation', 'bribe_logs', 'deed_transfers', 'deeds', 'executive_intake', 'executive_reports', 'secretary_assignments', 'secretary_tasks'],
  'games': ['wheel_inventory', 'troll_wheel_wins', 'games', 'game_players', 'internet_game_matches', 'matchmaking_queue', 'pitch_contests', 'pitches', 'pride_challenges', 'pride_user_progress', 'giveaways', 'daily_free_spins', 'daily_rewards', 'tip_packages', 'tips', 'music_tracks', 'music_tips', 'songs', 'song_likes'],
  'security': ['security_events', 'security_incident_reports', 'security_rate_limits', 'security_user_risk_scores', 'user_safety_warnings', 'user_ip_tracking', 'user_reputation', 'moderation_logs', 'moderation_evidence', 'web_push_subscriptions'],
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (trimmed.startsWith('--')) {
        if (inBlock && currentBlock.length > 0) {
          ...
          const lowerBlock = blockSql.toLowerCase();
          const hasTableRef = tables.some(t => lowerBlock.includes(`public.${t}`) || lowerBlock.includes(`"${t}"`) || lowerBlock.includes(`'${t}'`));
          if (hasTableRef) {
            pageSqlBlocks[page].push({
              sql: blockSql,
              source: file.name,
              line: i - currentBlock.length
            });
          }
        }
        currentBlock = [line];
...
for (const [page, blocks] of Object.entries(pageSqlBlocks)) {
  ...
  sql += `-- Tables: ${pageTables[page].slice(0, 10).join(', ')}${pageTables[page].length > 10 ? '...' : ''}\n`;
